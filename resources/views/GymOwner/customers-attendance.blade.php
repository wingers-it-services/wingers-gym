...
<script>
    ...
    function markStaffAttendance(input) {
        var gymId = parseInt(input.getAttribute("data-gym-id"));
        var staffIdElement = document.getElementById("staffId");

        if (!staffIdElement) {
            alert("Staff ID element not found!");
            return;
        }

        var employeeId = staffIdElement.value;
        var attendanceStatus = input.getAttribute("data-attendance-status");

        // Proceed with AJAX request
        $.ajax({
            url: "/mark-gym-user-attendance", // Define your update status route
            method: 'GET',
            headers: {
