...
class GymCustomerPaymentController extends Controller
{
    use SessionTrait;
    protected $gymSubscription;
    protected $gym; protected $gymUserSubscriptionsHistory;

    public function __construct(
        GymSubscription $gymSubscription,
        GymUserSubscriptionsHistory $gymUserSubscriptionsHistory,
        Gym $gym
    ) {
        $this->gymSubscription = $gymSubscription;
        $this->gymUserSubscriptionsHistory = $gymUserSubscriptionsHistory;
        $this->gym = $gym;
    }

    public function listGymCustomersSubscriptionPayment()
    {

        $gym_uuid = $this->getGymSession()['uuid'];
        $gymId = $this->gym->where('uuid', $gym_uuid)->first()->id;

        $subscriptionHistorys = $this->gymUserSubscriptionsHistory->where('gym_id', $gymId)->get();
        return view('GymOwner.gum-user-subscription-payment', compact('subscriptionHistorys'));
    }
